...
export default function Dashboard() {
  ...
  const [profileActions, setProfileActions] = useState<{[id:string]:string}>({})
  const [profileConfirmDelete, setProfileConfirmDelete] = useState<string|null>(null)
  ...
  // === ROLE GUARD: Universal portal protection ===
  useEffect(() => {
    async function guardPortal() {
      ...
      // User deleted from app_users but session alive → force logout
      if (!au) { await supabase.auth.signOut(); router.push('/'); return }
      // User disabled → force logout
      if (au.status === 'disabled') { await supabase.auth.signOut(); router.push('/'); return }
      // Job seeker on company dashboard → redirect to job seeker portal
      if (au.role === 'job_seeker') { router.push('/jobseeker'); return }
      // Platform admin on company dashboard → redirect to admin portal
      if (au.role === 'platform_admin') { router.push('/admin'); return }
    }
    guardPortal()
  }, [])
  // === END ROLE GUARD ===
  ...
  async function loadAll(u: any) {
    ...
    // === ROLE GUARD: Wrong portal? Redirect! ===
    if (au && au.role === 'job_seeker') { router.push('/jobseeker'); return }
    if (au && au.role === 'platform_admin') { router.push('/admin'); return }
    setAppUser(au)
  ...
  async function toggleProfileStatus(profileId: string, currentStatus: string) {
    const newStatus = (currentStatus || 'active') === 'active' ? 'disabled' : 'active'
    setProfileActions(prev => ({...prev, [profileId]: 'toggling'}))
    const { error } = await supabase.from('profiles').update({ status: newStatus }).eq('id', profileId)
    setProfileActions(prev => { const n = {...prev}; delete n[profileId]; return n })
    if (error) { alert('Failed: ' + error.message); return }
    setProfiles(prev => prev.map(p => p.id === profileId ? {...p, status: newStatus} : p))
  }

  async function updateProfilePlan(profileId: string, plan: string) {
    if (appUser?.role !== 'super_admin') { alert('Only Super Admin can change plans'); return }
    const { error } = await supabase.from('profiles').update({ plan }).eq('id', profileId)
    if (error) { alert('Failed: ' + error.message); return }
    setProfiles(prev => prev.map(p => p.id === profileId ? {...p, plan} : p))
  }

  async function bulkProfileAction(action: string) {
    if (!selectedProfiles.length) return
    if (action === 'delete') {
      const { error } = await supabase.from('profiles').delete().in('id', selectedProfiles)
      if (error) { setProfileConfirmDelete(null); alert('Delete failed: ' + error.message); return }
      setProfiles(prev => prev.filter(p => !selectedProfiles.includes(p.id)))
    } else {
      const newStatus = action === 'disable' ? 'disabled' : 'active'
      const { error } = await supabase.from('profiles').update({ status: newStatus }).in('id', selectedProfiles)
      if (error) { alert('Failed: ' + error.message); return }
      setProfiles(prev => prev.map(p => selectedProfiles.includes(p.id) ? {...p, status: newStatus} : p))
    }
    setSelectedProfiles([])
    setProfileConfirmDelete(null)
  }
  ...
  return (
    <Layout appUser={appUser} unreadCount={unreadCount}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'16px 20px'}}>
        ...
        {appUser?.role==='super_admin' && companies.length>0 && (
          <div style={{marginBottom:10,display:'flex',alignItems:'center',gap:10,flexWrap:'wrap' as const}}>
            <span style={{fontSize:12,color:'var(--mu)',fontWeight:600}}>🏢 View Company:</span>
            <select value={filterCompany} onChange={e=>setFilterCompany(e.target.value)} style={{background:'var(--bg3)',border:'1px solid var(--bd)',borderRadius:8,padding:'6px 12px',color:'var(--tx)',fontSize:12,fontFamily:'inherit',outline:'none'}}>
              <option value="">All Companies</option>
              {companies.map(c=><option key={c.id} value={c.id}>{c.name} ({c.company_code})</option>)}
            </select>
            {filterCompany&&<button onClick={()=>setFilterCompany('')} style={{fontSize:11,color:'#ff6b6b',background:'none',border:'none',cursor:'pointer',fontFamily:'inherit'}}>✕ Clear</button>}
          </div>
        )}
        {profileConfirmDelete && (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div style={{background:'var(--bg2)',border:'1px solid rgba(255,80,80,0.2)',borderRadius:20,padding:36,maxWidth:380,width:'90%',textAlign:'center' as const}}>
              <div style={{fontSize:44,marginBottom:12}}>🗑️</div>
              <div style={{fontSize:18,fontWeight:700,marginBottom:8}}>Confirm Delete</div>
              <div style={{color:'var(--mu)',fontSize:13,marginBottom:6}}>{profileConfirmDelete==='bulk'?'Delete '+selectedProfiles.length+' profile(s)?':'Delete this profile?'}</div>
              <div style={{color:'#ff5050',fontSize:12,marginBottom:28}}>This cannot be undone.</div>
              <div style={{display:'flex',gap:12,justifyContent:'center'}}>
                <button onClick={()=>setProfileConfirmDelete(null)} style={{padding:'8px 20px',borderRadius:10,background:'rgba(255,255,255,0.06)',color:'var(--mu)',border:'none',cursor:'pointer',fontFamily:'inherit'}}>Cancel</button>
                <button onClick={()=>bulkProfileAction('delete')} style={{padding:'8px 20px',borderRadius:10,background:'#ff3333',color:'#fff',border:'none',cursor:'pointer',fontFamily:'inherit',fontWeight:700}}>Yes, Delete</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{display:'flex',gap:8,marginBottom:14,alignItems:'center',flexWrap:'wrap' as const}}>
          ...
          {selectedProfiles.length>0 && <>
            ...
            {['super_admin','account_owner'].includes(appUser?.role) && <>
              <button onClick={()=>bulkProfileAction('enable')} style={{padding:'7px 12px',borderRadius:8,background:'rgba(61,214,140,0.12)',color:'#3dd68c',border:'1px solid rgba(61,214,140,0.3)',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:'inherit'}}>✅ Enable</button>
              <button onClick={()=>bulkProfileAction('disable')} style={{padding:'7px 12px',borderRadius:8,background:'rgba(255,159,67,0.12)',color:'#ff9f43',border:'1px solid rgba(255,159,67,0.3)',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:'inherit'}}>⏸ Disable</button>
              <button onClick={()=>setProfileConfirmDelete('bulk')} style={{padding:'7px 12px',borderRadius:8,background:'rgba(255,80,80,0.1)',color:'#ff5050',border:'1px solid rgba(255,80,80,0.3)',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:'inherit'}}>🗑️ Delete {selectedProfiles.length}</button>
            </>}
                        ...
                        <tr key={p.id} className="rh" style={{borderBottom:'1px solid rgba(255,255,255,0.04)',cursor:'pointer'}}>
                          ...
                          <td style={{padding:tdPad}} onClick={e=>e.stopPropagation()}>
                            <div style={{display:'flex',gap:3}}>
                              ...
                              {['super_admin','account_owner'].includes(appUser?.role) && <>
                                <label title={(p.status||'active')==='active'?'Disable':'Enable'} style={{cursor:'pointer',display:'flex',alignItems:'center'}}>
                                  <input type="checkbox" checked={(p.status||'active')==='active'} disabled={!!profileActions[p.id]} onChange={()=>toggleProfileStatus(p.id,p.status||'active')} style={{accentColor:'#3dd68c',width:14,height:14,cursor:'pointer'}}/>
                                </label>
                                <button onClick={()=>deleteProfile(p.id)} style={{background:'rgba(255,80,80,0.1)',border:'1px solid rgba(255,80,80,0.25)',borderRadius:5,padding:'2px 7px',fontSize:10,color:'#ff5050',cursor:'pointer',fontFamily:'inherit'}}>🗑</button>
                              </>}
                              {appUser?.role==='super_admin' && (
                                <button onClick={()=>updateProfilePlan(p.id,(p.plan||'free')==='free'?'premium':'free')} style={{padding:'2px 7px',borderRadius:10,fontSize:9,fontWeight:700,border:'none',cursor:'pointer',background:(p.plan||'free')==='premium'?'rgba(255,214,10,0.2)':'rgba(100,100,120,0.15)',color:(p.plan||'free')==='premium'?'#ffd60a':'#7a7f90'}}>
                                  {(p.plan||'free')==='premium'?'★ PRO':'Free'}
                                </button>
                              )}
                            </div>
                          </td>
