// @ts-nocheck
// ════════════════════════════════════════════════════════════════
// DEDICATED ADD PROFILE PAGE  (standalone, full page — not a modal)
// Self-contained: does NOT import anything from master.tsx
// Route: /dashboard/add-profile
// On save → inserts into `profiles` → redirects to /dashboard/master
// ════════════════════════════════════════════════════════════════
...
export default function AddProfilePage() {
  ...
  // ── Helpers for array sections (work/edu/cert/achievement) ──
  ...
  async function saveProfile(addAnother=false) {
    ...
    if (data) {
      ...
      // Notify the assigned user (if assigned to someone else)
      ...
      setSaving(false)
      if (addAnother) {
        showSuccess('✅ Profile added! Form cleared for the next one.')
        setForm({...EMPTY_PROFILE, segment: form.segment})
    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',color:'var(--tx)',fontFamily:'Outfit,sans-serif',paddingBottom:90}}>
      <style>{`
        *{box-sizing:border-box;}
        @media (max-width:640px){[style*="grid-template-columns"]{grid-template-columns:1fr !important;}[style*="gridTemplateColumns"]{grid-template-columns:1fr !important;}input,select,textarea{font-size:16px !important;min-height:44px;}button{min-height:42px;}.leaflet-container{height:260px !important;}}
        *{box-sizing:border-box;}
      ...
      <div style={{position:'sticky',top:0,zIndex:50,background:'var(--bg)',borderBottom:'1px solid var(--bd)',padding:'14px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
        <div style={{display:'flex',alignItems:'center',gap:14}}>
          ...
          <div>
            <div style={{fontSize:17,fontWeight:700}}>Add New Profile</div>
            <div style={{fontSize:11,color:'var(--mu)',marginTop:2}}>Create a new candidate profile</div>
          </div>
        </div>
        <button onClick={()=>setShowCV(true)} style={{background:'var(--acbg)',color:'var(--ac)',border:'1px solid var(--bd2)',borderRadius:8,padding:'9px 16px',fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>📄 Upload CV to auto-fill</button>
      </div>
      ...
      <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:60,background:'var(--bg2)',borderTop:'1px solid var(--bd)',padding:'12px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,boxShadow:'0 -8px 24px rgba(0,0,0,0.15)'}}>
        ...
        <div style={{display:'flex',gap:10}}>
          <button onClick={()=>saveProfile(true)} disabled={saving} style={{padding:'10px 20px',borderRadius:10,background:'var(--acbg)',color:'var(--ac)',border:'1px solid var(--bd2)',cursor:'pointer',fontSize:13,fontWeight:600,fontFamily:'inherit',opacity:saving?0.6:1}}>{saving?'Saving...':'Save & Add Another'}</button>
          <button onClick={()=>saveProfile(false)} disabled={saving} style={{padding:'10px 28px',borderRadius:10,background:'var(--gn)',color:'#fff',border:'none',cursor:'pointer',fontSize:14,fontWeight:700,fontFamily:'inherit',opacity:saving?0.7:1}}>{saving?'⏳ Saving...':'✅ Save Profile'}</button>
        </div>
      </div>
