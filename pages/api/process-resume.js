...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      const prompt = `Extract info. Return ONLY valid JSON. Keys: name, mobile, email, location, experienceYears, experienceMonths, ctcLakhs, ctcThousands, noticePeriod, headline, summary, skills (comma separated). Text: ${pdfData.text.substring(0, 15000)}`;
