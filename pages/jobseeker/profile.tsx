...
@media (max-width:380px){
  .jsp-hero-actions button{font-size:11px}
      ...
      <div className="jsp-wrap">
        ...
        <div className="jsp-hero" style={{ borderRadius: 18, padding: '20px 22px', marginBottom: 16, background: `linear-gradient(135deg, ${T.acbg}, transparent)`, border: `1px solid ${T.bd}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div className="jsp-hero-actions" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button onClick={() => setShowResume(true)} style={{ padding: '8px 14px', borderRadius: 10, background: T.acbg || 'rgba(99,102,241,0.12)', color: T.ac || '#6366f1', border: `1px solid ${T.ac || '#6366f1'}`, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: 12 }}>✨ Resume PDF</button>
      ...
      {showResume && <ResumeBuilder profile={{ ...form, photo_url: form.photo_url || user?.photo_url, email: form.email || user?.email }} onClose={() => setShowResume(false)} />}
    </div>
  )
}
