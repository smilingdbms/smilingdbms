    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', height: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ margin: 0, color: '#9CA3AF', fontSize: '13px', marginTop: '5px' }}>Manage all client agencies and their isolated databases.</p>
        </div>
      ...
      <div style={{ background: '#11182D', borderRadius: '12px', border: '1px solid #1F2937', overflow: 'auto', flex: 1 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          ...
          <tbody>
            {tenants.length === 0 ? (
              <tr><td colSpan="5" style={{ padding: '20px', textAlign: 'center', color: '#6B7280' }}>No consultancies onboarded yet.</td></tr>
            ) : (
              tenants.map(t => (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#11182D', width: '450px', borderRadius: '12px', border: '1px solid #374151', padding: '25px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={() => setIsModalOpen(false)} style={{ background: 'none', border: 'none', color: '#9CA3AF', cursor: 'pointer', fontSize: '20px' }}>✕</button>
            </div>
            
            <form onSubmit={handleAddTenant} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div>
                <label style={{ display: 'block', color: '#9CA3AF', fontSize: '12px', marginBottom: '5px' }}>Company Name</label>
                <input required type="text" value={formData.company_name} onChange={e => setFormData({...formData, company_name: e.target.value})} style={{ width: '100%', padding: '10px', background: '#050810', border: '1px solid #374151', color: '#fff', borderRadius: '6px' }} placeholder="e.g. Apex Staffing" />
              </div>
              <div>
                <label style={{ display: 'block', color: '#9CA3AF', fontSize: '12px', marginBottom: '5px' }}>Unique Company Code</label>
                <input required type="text" value={formData.company_code} onChange={e => setFormData({...formData, company_code: e.target.value.toUpperCase()})} style={{ width: '100%', padding: '10px', background: '#050810', border: '1px solid #374151', color: '#fff', borderRadius: '6px' }} placeholder="e.g. APEX01" />
              </div>
              <div>
                <label style={{ display: 'block', color: '#9CA3AF', fontSize: '12px', marginBottom: '5px' }}>Account Owner (AO) Name</label>
                <input required type="text" value={formData.owner_name} onChange={e => setFormData({...formData, owner_name: e.target.value})} style={{ width: '100%', padding: '10px', background: '#050810', border: '1px solid #374151', color: '#fff', borderRadius: '6px' }} placeholder="e.g. Rahul Sharma" />
              </div>
              <div>
