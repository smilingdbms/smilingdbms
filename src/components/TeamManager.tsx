...
export default function TeamManager() {
  ...
  const getRoleBadge = (role) => {
    const c = { super_admin:'#EF4444', account_owner:'#F59E0B', team_manager:'#818CF8', team_leader:'#A78BFA', sr_recruiter:'#34D399', recruiter:'#10B981', individual_recruiter:'#06B6D4', bd:'#60A5FA', job_seeker:'var(--mu)' };
    const color = c[role] || 'var(--mu)';
  ...
  const statusColor = { active:'#10B981', pending:'#F59E0B', disabled:'#EF4444', rejected:'var(--mu2)' };

  if (loading) return <div style={{color:'var(--mu2)',padding:20}}>Loading team...</div>;
    ...
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <div>
          ...
          <p style={{margin:'4px 0 0',color:'var(--mu2)',fontSize:13}}>{filtered.length} of {members.length} members</p>
        </div>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name or email..." style={{background:'var(--bg2)',border:'1px solid var(--bg3)',borderRadius:8,padding:'8px 14px',color:'#fff',fontSize:13,width:250,outline:'none'}} />
      </div>
      ...
      <div style={{display:'flex',gap:6,marginBottom:16,flexWrap:'wrap'}}>
        {STATUS_TABS.map(s => (
          <button key={s} onClick={()=>setStatusFilter(s)} style={{
            ...
            color: statusFilter===s ? (statusColor[s]||'#3B82F6') : 'var(--mu2)',
            borderColor: statusFilter===s ? (statusColor[s]||'#3B82F6')+'60' : 'var(--bg3)',
          }}>
      ...
      {/* Table */}
      <div style={{background:'var(--bg2)',borderRadius:12,border:'1px solid var(--bg3)',overflow:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',minWidth:700}}>
          <thead style={{background:'var(--bg3)'}}>
            <tr>
              {['Member','Role','Status','Joined','Last Login','Actions'].map(h=>(
                <th key={h} style={{padding:'12px 15px',textAlign:h==='Actions'?'right':'left',fontSize:11,color:'var(--mu)',textTransform:'uppercase',whiteSpace:'nowrap'}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} style={{padding:40,textAlign:'center',color:'var(--mu2)'}}>
                {search ? 'No matches found' : `No ${statusFilter==='all'?'':statusFilter+' '}members`}
              </td></tr>
            ) : filtered.map(m => (
              <tr key={m.id} style={{borderBottom:'1px solid var(--bg3)',background:m.status==='pending'?'rgba(245,158,11,0.03)':'transparent'}}>
                {/* Member */}
                <td style={{padding:12}}>
                  <div style={{display:'flex',alignItems:'center',gap:10}}>
                    <div style={{width:34,height:34,borderRadius:'50%',background:statusColor[m.status]||'var(--bg4)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,fontSize:13,color:'#fff',flexShrink:0,opacity:0.8}}>
                      {(m.full_name||m.email||'U')[0].toUpperCase()}
                    </div>
                    <div>
                      <div style={{fontSize:13,fontWeight:700,color:'#fff'}}>{m.full_name||'No name'}</div>
                      <div style={{fontSize:11,color:'var(--mu2)'}}>{m.email}</div>
                      {m.designation && <div style={{fontSize:10,color:'var(--bg4)'}}>{m.designation}</div>}
                    </div>
                  </div>
                </td>
                {/* Role */}
                <td style={{padding:12}}>
                  {editingRole === m.id ? (
                    <select value={m.role} onChange={e=>handleRoleChange(m.id,e.target.value)} onBlur={()=>setEditingRole('')} autoFocus style={{background:'var(--bg3)',color:'#fff',border:'1px solid var(--bg4)',borderRadius:6,padding:'4px 8px',fontSize:12}}>
                ...
                {/* Status */}
                <td style={{padding:12}}>
                  <span style={{color:statusColor[m.status]||'var(--mu)',fontSize:12,fontWeight:600}}>● {(m.status||'active').charAt(0).toUpperCase()+(m.status||'active').slice(1)}</span>
                </td>
                {/* Joined */}
                <td style={{padding:12,fontSize:12,color:'var(--mu2)',whiteSpace:'nowrap'}}>
                  {new Date(m.created_at).toLocaleDateString('en-IN')}
                </td>
                {/* Last Login */}
                <td style={{padding:12,fontSize:11,color:'var(--bg4)',whiteSpace:'nowrap'}}>
                ...
                <td style={{padding:12,textAlign:'right',whiteSpace:'nowrap'}}>
                  {m.id === currentUser?.id ? (
                    <span style={{fontSize:11,color:'var(--bg4)'}}>You</span>
                  ) : m.role === 'super_admin' && !isSA ? (
                    <span style={{fontSize:11,color:'var(--bg4)'}}>—</span>
                  ) : canManage ? (
                    <div style={{display:'flex',gap:6,justifyContent:'flex-end'}}>
                      ...
                      {m.status === 'active' && (
                        <button onClick={()=>handleStatusChange(m.id,'disabled','disable')} disabled={processing===m.id} style={{background:'rgba(239,68,68,0.08)',border:'1px solid var(--bg4)',color:'#EF4444',padding:'5px 12px',borderRadius:6,cursor:'pointer',fontSize:11}}>
                          {processing===m.id?'...':'Disable'}
                        </button>
                      )}
                      {(m.status === 'disabled' || m.status === 'rejected') && (
                        <button onClick={()=>handleStatusChange(m.id,'active','reactivate')} disabled={processing===m.id} style={{background:'rgba(16,185,129,0.1)',border:'1px solid var(--bg4)',color:'#10B981',padding:'5px 12px',borderRadius:6,cursor:'pointer',fontSize:11}}>
                          {processing===m.id?'...':'Activate'}
                        </button>
                      )}
                      <button onClick={()=>handleDelete(m.id,m.full_name||m.email)} disabled={processing===m.id} style={{background:'transparent',border:'1px solid var(--bg4)',color:'var(--mu2)',padding:'5px 10px',borderRadius:6,cursor:'pointer',fontSize:13}} title="Delete">
                        🗑️
                      </button>
                    </div>
                  ) : (
